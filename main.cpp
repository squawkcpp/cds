...
int main(int argc, char* argv[]) {
    ...
    _container.www->bind( http::mod::Match<std::string>( "^\\/+(root|file|[[:digit:]]+)\\/+nodes$", "module" ),
        http::mod::Exec( std::bind( &cds::Server::nodes, _container.server, _1, _2 ) ),
        http::mod::Http() );

    _container.www->bind( http::mod::Match<std::string>( "^\\/+(ebook|movie|album|serie|artist|image)\\/+nodes$", "module" ),
